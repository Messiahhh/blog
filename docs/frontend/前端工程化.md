...
对于绝大多数命令行程序，如`babel`、`webpack`、`eslint`等，使用的场合我们都需要传递自定义的参数，我们当然可以直接在命令行传递对应的参数，但更好的是在特定的配置文件中记录这些参数。

通常配置文件的格式都是相似的，比如`babel.config.js`、`webpack.config.js`；又或者`.babelrc`、`.eslintrc`
...
以前的一种模块化方案，类似的还有CMD。需要额外安装`require.js`库，并使用`define`定义模块，使用`require`加载模块。
...
又因为我们模块实际返回的是`module.exports`，所以如果对`exports`重新赋值肯定是错误的操作。
...
上述已经讲了三个模块化方案，而UMD就是它们的总和，也就是用来兼容多套模块化系统。



##### ES6模块

无论是浏览器还是Node环境都不支持直接使用`es模块`的写法，通常我们都需要使用`babel`来使用`es模块`。

或者在Node环境下设置`--experimental-modules`来使用`es模块`，此时文件的后缀是`.mjs`

``` javascript
node --experimental-modules file.mjs
```

`es6`中使用`export`和`import`来实现导出和导入，`export`用来定义模块对外的接口。
...
该代码中，`export default name`等价于`export { name as default}`，而`import name`等价于`import { default as name} from ''`，可以很清晰的感受到我们在定义模块的接口。
...
除此之外，我们还可以使用`import * as xx from ''`的写法来加载整个模块。
