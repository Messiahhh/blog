...
ctx.strokeRect(0, 0, canvas.width, canvas.height) // 清除整个画布内容
```
...
我们之前提到过Canvas的2D上下文本质是一个状态机，他还提供了保存和恢复当前状态的能力（以一种栈的形式）。我们可以先把当前状态保存，然后再进行坐标转换操作或者其他属性的修改，待绘制结束后再恢复到最开始的状态。
