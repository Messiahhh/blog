...
`Typed Array`可以理解成一种抽象类，具体的实现包括`Uint8Array`、`Int8Array`、`Uint32Array`、`Int32Array`等。
...
view.setUint16(0, 255, true) // 也可以使用小端字节序 
typedArray1[0] === 255 // true 
typedArray1[1] === 0 // true
```
...
:::caution 注意
可以注意到以上编码方式的结果和下面的结果不同，上面的TextEncoder和Buffer.from都采用的UTF-16进行编码，而JavaScript中字符串是通过UTF-16编码的，所以调用charCodeAt实际是进行了UTF-16编码。
``` js
