...
### OffscreenCanvas
我们的主页面存在着互斥的JS线程（通常也被称为JS线程）和渲染线程。每当JS线程执行完一个JS任务，就会切到渲染线程执行渲染任务（即屏幕内容的绘制），绘制完后又回到JS线程执行下一个JS任务。
...
Konva提供了一棵由虚拟节点组成的树，每当我们修改到数据源（添加节点、删除节点、修改属性等），都会执行`Layer`
